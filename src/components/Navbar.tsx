    ...
    >
      ...
      >
        ...
        <span className="font-cormorant font-bold text-[18.5px] text-white tracking-[0.5px]">
          SKILLS
          <sup className="text-[9px] text-gold3 align-super ml-[2px]">™</sup>
        </span>
      </Link>
      ...
      <div className="hidden xl:flex items-center gap-[3px]">
        {navLinks.map((link) => (
          <Link
            ...
            className={`text-[13px] font-medium font-outfit px-[13px] py-[6px] rounded-[6px] transition-all duration-200 ${
              pathname === link.href
                ? "text-gold3"
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <span
            className={`block w-[22px] h-[1.5px] bg-white transition-all ${isMenuOpen ? "rotate-45 translate-y-[6.5px]" : ""}`}
          />
          <span
            className={`block w-[22px] h-[1.5px] bg-white transition-all ${isMenuOpen ? "opacity-0" : ""}`}
          />
          <span
            className={`block w-[22px] h-[1.5px] bg-white transition-all ${isMenuOpen ? "-rotate-45 -translate-y-[6.5px]" : ""}`}
          />
        ...
        <div className="xl:hidden fixed top-[62px] left-0 right-0 bg-[rgba(11,31,58,0.98)] backdrop-blur-[24px] border-b border-[rgba(196,136,14,0.15)] py-6 px-5 flex flex-col gap-1 transition-all animate-in slide-in-from-top-2 duration-300">
          {navLinks.map((link) => (
            <Link
              ...
              className={`text-[14.5px] font-medium font-outfit p-3.5 rounded-lg transition-all ${
                pathname === link.href
